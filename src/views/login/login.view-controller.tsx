    ...
    <main className='ink-wash grid min-h-screen place-items-center overflow-hidden bg-neutral-950 px-4 py-10 text-neutral-200'>
      <div className='pointer-events-none fixed inset-0 z-0 overflow-hidden'>
        <GameImage
          ...
          src={inkLandscape}
          visibleByDefault
          wrapperClassName='absolute left-1/2 top-[-4rem] block w-[92rem] max-w-none -translate-x-1/2 opacity-[0.12]'
        />
        <GameImage
          alt='登入頁山水底紋'
          className='h-auto w-full'
          src={inkLandscape}
          visibleByDefault
          wrapperClassName='absolute bottom-[-10rem] right-[-18rem] block w-[66rem] max-w-none opacity-[0.1]'
        />
      </div>
      ...
      <section className='relative z-10 w-full max-w-[420px] rounded-lg border border-white/15 bg-neutral-950/65 p-6 shadow-2xl shadow-black/40 backdrop-blur-md sm:p-7'>
        <div className='mb-6 text-center'>
          ...
          <h1 className='mt-2 font-serif text-3xl text-neutral-100'>{isRegister ? '開立洞府' : '入境修行'}</h1>
          <p className='mt-2 text-sm text-neutral-400'>{isRegister ? '建立 mock 修士資料' : '使用 mock 帳號登入'}</p>
        </div>

        <div className='mb-6 grid grid-cols-2 rounded-full border border-white/12 bg-black/30 p-1'>
          <button
            className={cx(
              'rounded-full px-4 py-2 text-sm transition hover:bg-white/8',
        ...
        <form className='space-y-4' onSubmit={handleSubmit}>
          <label className='block'>
            <span className='mb-2 block text-xs text-neutral-500'>帳號 / Email</span>
          ...
          {isRegister ? (
            <label className='block'>
              <span className='mb-2 block text-xs text-neutral-500'>角色名稱</span>
              <div className='flex items-center gap-3 rounded-md border border-white/12 bg-black/30 px-3 py-3 focus-within:border-white/25'>
        ...
        <p className='mt-4 min-h-5 text-center text-xs text-neutral-500'>{message}</p>
      </section>
    </main>
  )
}
