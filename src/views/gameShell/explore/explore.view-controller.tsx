...
import { bind, getMapStatusPresentation } from '@/utils'
import { useExploreViewModel, type IExploreViewModel } from './explore.view-model'

/** 呈現地圖選擇、探索提交與戰鬥結果畫面。 */
export function exploreViewController({
  character,
  maps,
  selectedMap,
  isResultOpen,
  ...
  return (
    <>
            ...
            {maps.map((map) => {
              ...
              return (
                <button
                  ...
                  } ${map.status === 'locked' ? 'cursor-not-allowed opacity-55' : ''}`}
                ...
                >
                  <span className='flex min-w-0 items-start justify-between gap-3'>
                    <span className='min-w-0'>
                      <span className='block truncate font-serif text-lg text-neutral-100'>{map.name}</span>
                      <span className='mt-1 block text-xs text-neutral-500'>建議境界・{map.recommendedRealm}</span>
                    </span>
                    <StatusBadge tone={status.tone}>{status.label}</StatusBadge>
                  </span>

                  <span className='mt-3 block text-xs text-neutral-500'>{status.description}</span>
                  <span className='mt-3 grid gap-1 text-xs leading-5 text-neutral-600 sm:grid-cols-2'>
                    ...
                    <span className='min-w-0 break-words'>掉落倍率：×{map.dropMultiplier.toFixed(2)}</span>
                  </span>
                </button>
              )
            })}
        ...
        <Panel eyebrow='EXPEDITION READY' title='出發準備'>
          <div className='grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end'>
            <div className='min-w-0'>
              <p className='font-serif text-xl text-neutral-100'>{selectedMap?.name ?? '尚未選擇地圖'}</p>
              ...
              {selectedMap?.status === 'challenging' ? (
                <p className='mt-3 text-xs leading-6 text-gold-100'>越階探索將受到境界壓制，但稀有掉落機率提高。</p>
              ) : null}
              {hasLowResources ? (
                <p className='mt-3 text-xs leading-6 text-cinnabar-100' role='alert'>
                  目前生命或靈力偏低；探索仍可提交，戰鬥與獎勵由 後端結算。
                </p>
              ) : null}
            </div>
      ...
      <Modal
        ...
        headerAccessory={
          <StatusBadge
            tone={!canRevealBattleOutcome ? 'gold' : isEncounter || battle?.result === 'victory' ? 'jade' : 'cinnabar'}
          >
            {!canRevealBattleOutcome
        ...
        title={canRevealBattleOutcome ? (resultView?.title ?? '探索結果') : '戰鬥推演'}
      >
        {isEncounter ? (
          <div className='my-5 grid flex-1 place-items-center rounded-lg border border-jade-400/20 bg-jade-400/[0.05] p-6 text-center'>
            <div>
              <p aria-hidden='true' className='font-serif text-6xl text-jade-100'>
                緣
              </p>
              <p className='mt-5 text-lg text-neutral-200'>{resultView?.eventMessage}</p>
          ...
          <div
            className={`grid min-h-0 flex-1 gap-4 py-5 ${
              isOutcomeRevealed ? 'lg:grid-cols-[minmax(0,1.5fr)_minmax(18rem,0.7fr)]' : ''
            }`}
          >
            <section className='flex min-h-0 min-w-0 flex-col rounded-lg border border-white/12 bg-ink-900/70 p-4 sm:p-5'>
              <div className='flex flex-wrap items-center justify-between gap-2'>
                <div>
                  <h3 className='font-serif text-lg text-neutral-100'>戰鬥推演</h3>
                  <p aria-live='polite' className='mt-1 text-xs text-neutral-500' role='status'>
                ...
                <StatusBadge tone={isOutcomeRevealed ? 'jade' : 'gold'}>
                  {isOutcomeRevealed ? '已結算' : battlePlaybackPhase === 'settling' ? '結算中' : '進行中'}
                </StatusBadge>
              </div>
              <ol
                ...
                {visibleBattleLog.map((entry, index) => (
                  ...
                  >
                    <div className='flex flex-wrap items-center gap-2 text-xs'>
                      <span className='text-gold-100'>第 {entry.round} 回合</span>
                      <span className={entry.hit ? 'text-jade-100' : 'text-neutral-500'}>
                        {entry.hit ? '命中' : '閃避'}
                      </span>
                      {entry.critical ? <span className='text-cinnabar-100'>暴擊</span> : null}
                    </div>
                    <p className='mt-1 text-neutral-300'>{entry.message}</p>
                    <p className='mt-1 text-xs tabular-nums text-neutral-600'>
                      {entry.hit ? `造成 ${entry.damage.toLocaleString()} 點傷害` : '本次未造成傷害'}
                      <span aria-hidden='true'> ・ </span>
                      <span className='font-semibold text-gold-100'>{entry.targetName}</span> 剩餘生命{' '}
                      <strong className='font-semibold text-cinnabar-100'>{entry.targetHp.toLocaleString()}</strong>
                    </p>
                  </li>
                ))}
              </ol>
            ...
            {isOutcomeRevealed ? (
              <aside className='grid min-w-0 content-start gap-4'>
                <section className='rounded-lg border border-white/12 bg-ink-900/70 p-4'>
                  <h3 className='font-serif text-lg text-neutral-100'>結算狀態</h3>
                  <dl className='mt-3 grid grid-cols-2 gap-2 text-xs'>
                    {[
                      ...
                      [
                        '戰鬥結束生命',
                        battle?.healthRemaining === undefined ? '—' : battle.healthRemaining.toLocaleString(),
                      ],
                      [
                        '戰鬥結束靈力',
                        battle?.spiritRemaining === undefined ? '—' : battle.spiritRemaining.toLocaleString(),
                      ],
                      [
                        '敵方生命',
                        battle?.enemyHealthRemaining === undefined ? '—' : battle.enemyHealthRemaining.toLocaleString(),
                      ],
                      ['狀態', battle?.result === 'victory' ? '完成' : '未完成'],
                    ].map(([label, value]) => (
                      <div className='min-w-0 rounded border border-white/10 bg-black/20 p-2.5' key={label}>
                        <dt className='truncate text-neutral-600'>{label}</dt>
                        <dd className='mt-1 break-words tabular-nums text-neutral-200'>{value}</dd>
                      </div>
                    ))}
                  </dl>
                  ...
                  {battle?.settledHealthRemaining !== undefined || battle?.settledSpiritRemaining !== undefined ? (
                    <div className='mt-3 rounded border border-cinnabar-400/20 bg-cinnabar-400/[0.05] p-3 text-xs leading-5 text-neutral-400'>
                      <p className='font-medium text-cinnabar-100'>戰敗返回洞府後</p>
                      <p className='mt-1 tabular-nums'>
                        生命 {battle.settledHealthRemaining?.toLocaleString() ?? '—'}
                        {' ・ '}靈力 {battle.settledSpiritRemaining?.toLocaleString() ?? '—'}
                      </p>
                      <p className='mt-1 text-neutral-600'>戰敗善後會保留 30% 生命與靈力，此數值不同於戰鬥停止當下。</p>
                    </div>
                  ) : null}
                </section>

                <section className='rounded-lg border border-gold-400/20 bg-gold-400/[0.05] p-4'>
                  <h3 className='font-serif text-lg text-neutral-100'>獎勵摘要</h3>
                  {battle?.rewards.length ? (
                    ...
                    <p className='mt-3 text-sm text-neutral-600'>本次探索沒有獎勵。</p>
                  )}
                </section>
              </aside>
            ) : null}
        ...
        <footer className='mt-auto grid gap-2 border-t border-white/12 pt-4 sm:grid-cols-2'>
          <Button onClick={handleCloseResult} variant='ghost'>
            返回探索
          </Button>
          {canRevealBattleOutcome && (resultView?.createdEquipmentIds.length ?? 0) > 0 ? (
            <Button onClick={handleGoToLoadout}>前往整備</Button>
          ) : null}
        </footer>
      </Modal>
    </>
...
export default bind(exploreViewController, useExploreViewModel)
