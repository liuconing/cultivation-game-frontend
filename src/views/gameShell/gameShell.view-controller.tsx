...
export function gameShellViewController({
  ...
  return (
    <div className='ink-wash min-h-dvh bg-ink-950 text-neutral-200'>
      ...
      <div className='min-h-dvh md:pl-56'>
        <header className='sticky top-0 z-30 border-b border-white/12 bg-ink-950/94 px-3 py-3 backdrop-blur-xl sm:px-5'>
          <div className='mx-auto flex max-w-7xl flex-col gap-2'>
              ...
              >
                ...
                <span className='min-w-0'>
                  ...
                  <span className='block truncate text-[0.7rem] text-neutral-500'>
                    {character.realm}・{character.minorRealm} · {character.spiritualRoot}・
              ...
              <div className='relative shrink-0'>
                ...
                {isAccountMenuOpen ? (
                  <div
                    ...
                    role='menu'
                  >
                    <p className='px-3 py-2 text-xs text-neutral-600'>{accountLabel}</p>
                    <button
            ...
            <div className='grid min-w-0 grid-cols-3 gap-1.5 sm:grid-cols-[1.35fr_0.75fr_0.75fr_0.75fr] sm:gap-2'>
              <div className='col-span-3 min-w-0 rounded-md border border-white/10 bg-black/20 px-2.5 py-1.5 sm:col-span-1'>
                <div className='flex items-center justify-between gap-2 text-[0.65rem]'>
                  ...
                  <span className='shrink-0 tabular-nums text-gold-100'>
                    {formatCompactNumber(character.cultivation)}／{formatCompactNumber(character.cultivationTarget)}
                  </span>
                </div>
                <div className='mt-1 h-1 overflow-hidden rounded-full bg-black/50'>
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (character.cultivation / character.cultivationTarget) * 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className='min-w-0 rounded-md border border-cinnabar-400/18 bg-cinnabar-400/[0.06] px-2.5 py-1.5 text-[0.65rem]'>
                ...
                <span className='block truncate tabular-nums text-cinnabar-100'>
                  {formatCompactNumber(character.health)}／{formatCompactNumber(character.maxHealth)}
                </span>
              </div>
              ...
              <div className='min-w-0 rounded-md border border-jade-400/18 bg-jade-400/[0.06] px-2.5 py-1.5 text-[0.65rem]'>
                ...
                <span className='block truncate tabular-nums text-jade-100'>
                  {formatCompactNumber(character.spiritPower)}／{formatCompactNumber(character.maxSpiritPower)}
                </span>
              </div>
            ...
            <div aria-label='全域遊戲提示' className='flex min-h-7 min-w-0 items-center gap-1.5 overflow-x-auto'>
        ...
        >
          <div className='mb-4 flex flex-col gap-3 rounded-lg border border-white/10 bg-ink-900/50 p-3 sm:flex-row sm:items-center sm:justify-between'>
            <div className='min-w-0'>
              <p className='text-[0.68rem] tracking-[0.18em] text-gold-200/65'>GAME</p>
              <p className='truncate font-serif text-lg text-neutral-100'>{activeItem.label}</p>
            </div>
              ...
              <Panel eyebrow='CATALOG ERROR' title='物品資料載入失敗'>
                <div className='grid min-h-64 place-items-center rounded-md border border-dashed border-cinnabar-400/25 bg-cinnabar-400/[0.05] p-5 text-center'>
                  <div>
                    ...
                    <Button className='mt-4' onClick={handleReloadCatalog} variant='secondary'>
                      重新載入物品資料
                    </Button>
                  </div>
                </div>
              </Panel>
            ) : activeItem.path === '/game/cultivation' && !gameState.isLoading ? (
              <CultivationView />
            ) : activeItem.path === '/game/explore' && !gameState.isLoading ? (
              <ExploreView />
            ) : activeItem.path === '/game/loadout' && !gameState.isLoading ? (
              <LoadoutPage />
            ) : activeItem.path === '/game/cave' && !gameState.isLoading ? (
              <CaveView />
            ) : (
              <Panel eyebrow='PAGE STATUS' title={activeItem.label}>
                {gameState.isLoading ? (
                  <div
                  ...
                  >
                    <div className='text-center'>
                      ...
                      />
                      <p className='mt-3 text-sm text-neutral-500'>載入遊戲資料</p>
                    </div>
                  </div>
                ) : (
                  <div className='min-h-64 rounded-md border border-dashed border-white/12 bg-black/15 p-5 sm:p-7'>
                    <div className='flex items-start gap-4'>
                      ...
                      <div className='min-w-0'>
                        <h2 className='font-serif text-xl text-neutral-100'>{activeItem.label}資料暫時無法顯示</h2>
                        <p className='mt-2 max-w-2xl text-sm leading-7 text-neutral-400'>{activeItem.description}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <Panel eyebrow='CHARACTER RESOURCES' title='角色資源'>
              <dl className='grid gap-3 text-sm'>
                ...
                <div className='rounded-md border border-white/10 bg-black/15 p-3'>
                  ...
                  <dd className='mt-1 tabular-nums text-neutral-300'>第 {gameState.cultivationState.pity} 次</dd>
                </div>
                <div className='rounded-md border border-white/10 bg-black/15 p-3'>
      ...
      <Drawer isOpen={isCharacterDrawerOpen} onClose={handleCloseCharacterDrawer} title='角色詳情'>
        <div className='flex items-start justify-between gap-3'>
          <div className='min-w-0'>
            <p className='break-words font-serif text-2xl text-neutral-100'>{character.name}</p>
        ...
        <div className='mt-6 grid grid-cols-2 gap-2'>
          {[
            ...
            <div className='min-w-0 rounded-md border border-white/10 bg-black/20 p-3' key={label}>
              ...
              <p className='mt-1 break-words tabular-nums text-neutral-200'>{value}</p>
            </div>
          ))}
        </div>

        <div className='mt-6'>
          <ProgressBar label='修為進度' max={character.cultivationTarget} tone='gold' value={character.cultivation} />
        </div>

        <div className='mt-6 grid grid-cols-2 gap-2'>
          <div className='rounded-md border border-white/10 bg-white/[0.025] p-4'>
            <p className='text-xs text-neutral-600'>靈根精華</p>
            <p className='mt-2 tabular-nums text-gold-100'>{gameState.cultivationState.rootEssence.toLocaleString()}</p>
          </div>
          <div className='rounded-md border border-white/10 bg-white/[0.025] p-4'>
            <p className='text-xs text-neutral-600'>突破保底</p>
            <p className='mt-2 tabular-nums text-jade-100'>第 {gameState.cultivationState.pity} 次</p>
          </div>
        </div>
      </Drawer>
    </div>
