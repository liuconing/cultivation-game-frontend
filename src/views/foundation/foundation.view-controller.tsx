import { Button, Drawer, Modal, Panel, ProgressBar, StatusBadge, Tabs, TextField } from '@/components'
import { bind } from '@/utils'
import type { MockScenario } from '@/data/gameMock'
import { useFoundationViewModel, type IFoundationViewModel, type FoundationTab } from './foundation.view-model'

const foundationTabs: { value: FoundationTab; label: string }[] = [
...
export function foundationViewController({
  gameState,
  scenarioOptions,
  selectedTab,
  isModalOpen,
  ...
  return (
    <div className='ink-wash min-h-screen text-neutral-200'>
      <header className='border-b border-white/10 bg-ink-950/85 px-4 py-5 backdrop-blur-xl sm:px-6 lg:px-8'>
        <div className='mx-auto flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
          <div className='min-w-0'>
            <p className='text-xs tracking-[0.24em] text-gold-200/65'>UI-01・XIA-36</p>
            <h1 className='mt-2 font-serif text-3xl text-neutral-100 sm:text-4xl'>問仙・設計基礎</h1>
            <p className='mt-2 max-w-2xl text-sm leading-6 text-neutral-400'>
              共用元件、穩定尺寸與純記憶體 GameState 的互動展示板。
            </p>
          </div>
          <label className='block w-full sm:w-56'>
            <span className='mb-2 block text-xs text-neutral-500'>Fixture 情境</span>
            <select
      ...
      <main className='relative z-10 mx-auto grid max-w-7xl gap-5 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(19rem,0.65fr)] lg:px-8 lg:py-8'>
        <div className='grid min-w-0 gap-5'>
          <Panel eyebrow='DESIGN TOKENS' title='色彩、按鈕與狀態'>
            ...
            <div className='mt-5 grid grid-cols-2 gap-3 sm:flex sm:flex-wrap'>
              <Button className='w-full sm:w-auto' onClick={handleClaimCultivation}>
                領取修為
              </Button>
              <Button className='w-full sm:w-auto' onClick={handleOpenModal} variant='secondary'>
                開啟彈窗
              </Button>
              <Button className='w-full sm:w-auto' onClick={handleOpenDrawer} variant='ghost'>
                開啟抽屜
              </Button>
              <Button className='w-full sm:w-auto' disabled variant='danger'>
                資源不足
              </Button>
              <Button className='w-full sm:w-auto' isLoading variant='ghost'>
                載入
              </Button>
            </div>
          </Panel>
          ...
          <Panel eyebrow='FORM & TABS' title='輸入與頁籤'>
            <Tabs
              label='Foundation 分類'
              onChange={(value) => handleTabChange(value as FoundationTab)}
              options={[...foundationTabs]}
              value={selectedTab}
            />
            <div className='mt-5 grid gap-4 sm:grid-cols-2'>
              <TextField defaultValue='沈望舒' hint='姓名限制為 1 至 12 字；此處只展示欄位樣式。' label='角色姓名' />
              <TextField defaultValue='超過十二字的角色姓名示例' error='角色姓名不可超過 12 字。' label='錯誤狀態' />
            </div>
          </Panel>
          ...
          <Panel eyebrow='MOCK GAME STATE' title={character.name}>
            <div className='flex flex-wrap items-center justify-between gap-3'>
              ...
              <StatusBadge tone={scenarioTone[gameState.scenario]}>{gameState.scenario}</StatusBadge>
            </div>
            <div className='mt-5 grid gap-4'>
              <ProgressBar label='修為' max={character.cultivationTarget} value={character.cultivation} />
              <ProgressBar label='生命' max={character.maxHealth} tone='cinnabar' value={character.health} />
              <ProgressBar label='靈力' max={character.maxSpiritPower} tone='gold' value={character.spiritPower} />
            </div>
            <p
              aria-live='polite'
        ...
        <aside className='grid min-w-0 content-start gap-5'>
            ...
            <dl className='grid grid-cols-2 gap-3'>
              {[
                ...
                <div className='min-w-0 rounded-md border border-white/10 bg-black/20 p-3' key={label}>
                  ...
                  <dd className='mt-1 text-xl tabular-nums text-neutral-100'>{value}</dd>
                </div>
              ))}
            </dl>
          </Panel>
          ...
          <Panel eyebrow='MEMORY ONLY' title='資料原則'>
            ...
            <Button className='mt-5 w-full' onClick={handleReset} variant='ghost'>
              重設 Fixture
            </Button>
          </Panel>
        </aside>
      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title='突破確認範例'>
      ...
      <Drawer isOpen={isDrawerOpen} onClose={handleCloseDrawer} title='角色詳情抽屜範例'>
        <dl className='space-y-4'>
          ...
          <div>
            ...
            <dd className='tabular-nums'>{character.spiritStones.toLocaleString()}</dd>
          </div>
        </dl>
        <Button className='mt-6 w-full' onClick={handleCloseDrawer} variant='ghost'>
          關閉抽屜
        </Button>
      </Drawer>
    </div>
...
export default bind(foundationViewController, useFoundationViewModel)
