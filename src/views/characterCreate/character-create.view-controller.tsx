...
import {
  useCharacterCreateViewModel,
  type ICharacterCreateViewModel,
} from './character-create.view-model'
...
export function characterCreateViewController({
  ...
  const selectedRoot = spiritualRootOptions.find(
    (option) => option.value === values.spiritualRootType,
  )
  ...
  if (screen === 'result' && result) {
    const resultRoot = spiritualRootOptions.find(
      (option) => option.value === result.spiritualRootType,
    )
    const resultGender = genderOptions.find(
      (option) => option.value === result.gender,
    )
      ...
      <main className="ink-wash grid min-h-screen place-items-center bg-ink-950 px-4 py-8 text-neutral-200 sm:px-6">
        <section className="w-full max-w-2xl overflow-hidden rounded-lg border border-gold-300/25 bg-ink-900/85 shadow-2xl shadow-black/50">
          <div className="border-b border-white/10 bg-gradient-to-b from-gold-400/[0.08] to-transparent px-5 py-8 text-center sm:px-8 sm:py-10">
            ...
            <p className="mt-6 text-xs tracking-[0.24em] text-gold-200/65">
              SPIRITUAL ROOT AWAKENED
            </p>
            ...
            <h1 className="mt-5 break-words font-serif text-4xl text-neutral-100">
              {result.name}
            </h1>
            ...
            <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                >
                  ...
                  <dd className="mt-1 truncate text-sm text-neutral-100">
                    {value}
                  </dd>
                </div>
              ))}
    ...
    <main className="ink-wash min-h-screen bg-ink-950 text-neutral-200">
      <div className="mx-auto grid min-h-screen max-w-7xl lg:grid-cols-[minmax(20rem,0.7fr)_minmax(34rem,1.3fr)]">
        <aside className="border-b border-white/10 px-4 py-8 sm:px-8 lg:flex lg:flex-col lg:justify-between lg:border-b-0 lg:border-r lg:py-12">
          <div>
            <p className="text-xs tracking-[0.26em] text-gold-200/65">
              UI-03・XIA-38
            </p>
            <h1 className="mt-4 font-serif text-4xl text-neutral-100 sm:text-5xl">
              初入仙途
            </h1>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {spiritualRootOptions.map((option) => (
                    ...
                    >
                      ...
                      <span className="mt-2 block text-sm text-neutral-100">
                        {option.label}
                      </span>
                      <span className="mt-1 block truncate text-xs text-neutral-500">
                        {option.description}
